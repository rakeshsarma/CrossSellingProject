...
#boxplots
#boxplot1: Branch_Id vs revenue
marketing_data$Branch_ID<-as.factor(as.character(marketing_data$Branch_ID))
boxplot(marketing_data$Ticket_Revenue~marketing_data$Branch_ID)

#boxplot2: Customer_Type vs revenue
marketing_data$Customer_Type<-as.factor(as.character(marketing_data$Customer_Type))
boxplot(marketing_data$Ticket_Revenue~marketing_data$Customer_Type)
...
marketing_data_2 <- select(marketing_data_1, -c(Customer_Name:Contact, Ticket_Number,Current_Email, Week_Ending_Date, Schedule_Time, Dispatch_Time, Complete_Time))
#Sanity Check
...
View(marketing_data_5)
#F2:- Test_data - Difference between Last_Service_date and Setup_date: Customer_age
...
#---------------------------------
#F4 Train_Data - Have a newfeature to when a call was made, earlyMorning, Morning, evening, Night
call_time_category<-discretize(marketing_data_5$Call_Time, nbins = 4, disc = "equalwidth")
call_time_category <- apply(call_time_category, 2, as.factor)
marketing_data_5$call_time_category <- call_time_category
marketing_data_5$call_time_category <- as.factor(marketing_data_5$call_time_category)
#F5 Test_Data - Have a newfeature to when a call was made, earlyMorning, Morning, evening, Night
call_time_category_test<-discretize(marketing_data_test_2$Call_Time, nbins = 4, disc = "equalwidth")
call_time_category_test <- apply(call_time_category_test, 2, as.factor)
marketing_data_test_2$call_time_category <- call_time_category_test
marketing_data_test_2$call_time_category <- as.factor(marketing_data_test_2$call_time_category)
View(marketing_data_test_2)
...
marketing_data_7 <- select(marketing_data_6, -c(Branch_ID:Bill_To, Year_Month, Call_Time) )
rm(marketing_data_6)
...
marketing_data_test_4 <- select(marketing_data_test_3, -c(Branch_ID:Bill_To, Year_Month,Call_Time) )
...
marketing_data_7$call_time_category <- as.factor(as.numeric(marketing_data_7$call_time_category))
#sanity check
str(marketing_data_7)
...
marketing_data_test_4$call_time_category<-as.factor(as.character(marketing_data_test_4$call_time_category))
#sanity check
str(marketing_data_test_4)
...
#M3 : svm model on train data
formula<-as.formula(paste("diff~", paste(setdiff(colnames(marketing_data_7), "diff"), collapse = '+')))
x_variables<- setdiff(colnames(marketing_data_7), "diff")
svm_model<-svm(x =marketing_data_7[,x_variables], y = marketing_data_7$diff,data=marketing_data_7, 
    type = "eps-regression", kernel = "linear", cost=10, gamma = 0.1 )
...
sum(abs(test_data_with_preds$diff- test_data_with_preds$predictions_on_test))/sum(test_data_with_preds$diff)
